		...
		<table class="table shop_table">
			<thead>
				<tr>
					...
					<th><?php  echo "<div>" . vmText::_ ('COM_VIRTUEMART_CART_SUBTOTAL_TAX_AMOUNT') . '</div>' ?></th>
					<?php } ?>
				...
				<tr valign="top" class="sectiontableentry<?php echo $i ?>">
					...
					<td style="text-align: center;">
						...
						if (VmConfig::get ('checkout_show_origprice', 1) && $prow->prices['discountedPriceWithoutTax'] != $prow->prices['priceWithoutTax']) {
							echo '<div class="line-through">' . $this->currencyDisplay->createPriceDiv ('basePriceVariant', '', $prow->prices, TRUE, FALSE) . '</div><br />';
						}
					...
					<td ><?php
							...
							<div class="input-group">
						      ...
						      <div class="input-group-btn">
						        <button type="submit" class="vmicon vm2-add_quantity_cart btn btn-default btn-sm" name="updatecart.<?php echo $pkey ?>" title="<?php echo  vmText::_ ('COM_VIRTUEMART_CART_UPDATE') ?>" ><i class="fa fa-refresh"></i></button>
								<button type="submit" class="vmicon vm2-remove_from_cart btn btn-default btn-sm" name="delete.<?php echo $pkey ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CART_DELETE') ?>" ><i class="fa fa-times"></i></button>
						      </div>
						    </div><!-- /input-group -->
					</td>

					<?php if (VmConfig::get ('show_tax')) { ?>
					<td ><?php echo "<div class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('taxAmount', '', $prow->prices, FALSE, FALSE, $prow->quantity) . "</div>" ?></td>
					<?php } ?>
					<td ><?php echo "<div class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('discountAmount', '', $prow->prices, FALSE, FALSE, $prow->quantity) . "</div>" ?></td>
					<td colspan="1" >
						<?php
