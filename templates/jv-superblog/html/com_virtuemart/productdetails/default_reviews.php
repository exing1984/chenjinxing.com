...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	for( $num = 0; $num<=$maxrating; $num++ ) {
	  ...
	  echo '<div class="prodetail-review customer-reviews" id="review">';
...
if ($this->showReview) {
	...
	?>
	<h3 class="post-subtitle"><span><?php echo vmText::_ ('COM_VIRTUEMART_REVIEWS') ?> (<?php echo $reviews;?>)</span></h3>
				...
				foreach ($this->rating_reviews as $review) {
					...
					if (!empty($this->rating_reviews) && $review->published) {
						...
						<li>
							<div class="comment">
								...
								?>
								<div class="comment-avatar"> <img src="<?php echo $grav_url; ?>" alt="<?php echo $review->name;?>" width="80" onError="this.src='<?php echo JUri::base() ?>templates/jv-superblog/images/avatar.png';"  /> </div>
								<div class="comment-block">
									<div class="comment-head">
										<span><?php echo JText::_("TPL_BLOG_BY").' '.$review->customer; ?></span>
										<span><?php echo JHtml::date ($review->created_on, JText::_("TPL_PORTFOLIO_DATE_FORMAT")); ?></span>
									</div>
									<div class=""><?php echo $stars[(int)$review->review_rating] ?></div>
					...
					if ($i == $ratingsShow && !$showall) {
						...
						if ($reviews_published >= $ratingsShow) {
							$attribute = array('class'=> 'btn btn-primary pull-right btn-xs', 'title'=> vmText::_ ('COM_VIRTUEMART_MORE_REVIEWS'));
...
if ($this->allowRating or $this->allowReview) {
		...
		<div class="post-block post-leave-comment">
			<form method="post"
				...
				<?php if($this->allowRating and $review_editable) { ?>
					...
					<h3 class="mt-0"><?php echo vmText::_( 'COM_VIRTUEMART_WRITE_REVIEW' );
				...
				if($review_editable and $this->allowReview) { ?>
					<div class="row">
						<div class="col-md-12">
							<input class="btn btn-primary btn-sm pull-right" type="submit" onclick="return( check_reviewform());"
				...
				<?php } else if($review_editable and $this->allowRating) { ?>
					<div class="row">
						<div class="col-md-12">
							<input class="btn btn-primary btn-sm pull-right" type="submit" name="submit_review"
	...
	<?php
		...
		echo '<div class="post-block post-leave-comment text-center">';
		echo '<h3 class="post-subtitle"><span>'.vmText::_( 'COM_VIRTUEMART_DEAR' ).$this->user->name.'</span></h3>';
