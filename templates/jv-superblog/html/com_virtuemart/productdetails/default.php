...
<div class="productdetails-view productdetails pd-content">
	<div class="productdetails-content">
		<div class="row">
			<div class="col-sm-5 imagesProduct mb-0">
				<?php
					...
					<div class="review-price clearfix">
						<ul class="list-review list-unstyled">
							...
							<li>
								...
								?>
								<?php echo '('.$reviews.' '; echo ($reviews > 0)?JText::_('TPL_REVIEW_S'):JText::_('TPL_REVIEW'); echo ')'; ?>
	...
	<div class="tabs tabs-2">
		<ul class="nav nav-tabs clearfix">
			...
			<?php if (!empty($this->product->customfieldsSorted['normal'])) { ?>
				<li class="nav-icon <?php echo (empty($this->product->product_desc))?"active":""?>"><a href="#additional" data-toggle="tab" aria-expanded="true"><?php  echo JText::_( 'TPL_ADDITIONAL' ); ?></a></li>
			<?php } ?>
			<li class="nav-icon <?php echo (empty($this->product->product_desc) && empty($this->product->customfieldsSorted['normal']))?'active':'';?>">
	        	<?php
	        		$ratingsModel = VmModel::getModel ('ratings');
					$rating_reviews = $ratingsModel->getReviewsByProduct($this->product->virtuemart_product_id);
					$reviews = count($rating_reviews);
		...
		<div class="tab-content">
			...
			<?php if (!empty($this->product->customfieldsSorted['normal'])) { ?>
			<div class="tab-pane<?php echo (empty($this->product->product_desc))?" active":""?>" id="additional">
				<?php echo shopFunctionsF::renderVmSubLayout('customfields',array('product'=>$this->product,'position'=>'normal')); ?>
			</div>
			<?php } ?>
			<div class="tab-pane<?php echo (empty($this->product->product_desc) && empty($this->product->customfieldsSorted['normal']))?' active':'';?>" id="review">
