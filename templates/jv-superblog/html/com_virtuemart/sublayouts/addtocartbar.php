...
if (!VmConfig::get('use_as_catalog', 0)  ) { ?>
	...
	if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
		<a href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' . $product->virtuemart_product_id); ?>" class="notify btn btn-lg btn-default"><?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?></a><?php
	} else {
		$tmpPrice = (float) $product->prices['costPrice'];
		if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { ?>
			<?php
			if(!empty($addtoCartButton)){
				echo '<span class="btn-cartwrap">'.$addtoCartButton.'</span>';
			} 
			...
			if (VmConfig::get('ask_question', 0) == 1) {
				...
				?>
				<a class="ask btn btn-lg link-modal" href="<?php echo $askquestion_url ?>" rel="nofollow" title="<?php echo vmText::_('COM_VIRTUEMART_PRODUCT_ENQUIRY_LBL') ?>"><i class="fa fa-question-circle"></i></a>
			<?php
			}
			?>
	...
	if (VmConfig::get('ask_question', 0) == 1) {
		$askquestion_url = JRoute::_('index.php?option=com_virtuemart&view=productdetails&task=askquestion&virtuemart_product_id=' . $product->virtuemart_product_id . '&virtuemart_category_id=' . $product->virtuemart_category_id . '&tmpl=component', FALSE);
		?>
		<a class="ask link-modal btn btn-outline btn-default" href="<?php echo $askquestion_url ?>" rel="nofollow" title="<?php echo vmText::_('COM_VIRTUEMART_PRODUCT_ENQUIRY_LBL') ?>"><i class="fa fa-question-circle"></i></a>
	<?php
	}
}?>
